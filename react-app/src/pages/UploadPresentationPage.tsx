...
export function UploadPresentationPage() {
	...
	const handleFileSelect = async () => {
		...
		try {
			// Проверяем, доступны ли Electron API (десктоп версия)
			if (window.electronAPI) {
				// Десктоп версия через Electron
				const filePath = await window.electronAPI.selectFile()
				if (!filePath) return

				const fileBuffer: Buffer = await window.electronAPI.readFile(filePath)
				const fileNameFromPath =
					filePath.split(/[/\\]/).pop() || 'uploaded_file.pptx'
	...
	const handleConfirm = async () => {
		...
		try {
			...
			toast.success(
				`Лекция "${lectureName}" создана с ${selectedSlides.size} слайдами`
			)

			// Переходим на страницу настроек с реальным ID лекции
			navigate(`/settings/${lectureId}`)
		} catch (error) {
			console.error('Failed to create lecture:', error)
			toast.error('Ошибка при создании лекции')
		}
	}
		...
		<div className="p-4 sm:p-6 lg:p-8">
			<div className="mb-6">
				...
				<p className="text-sm text-neutral-500">
					Загрузите PowerPoint или PDF для создания новой лекции
				</p>
			</div>
			...
			{parsedSlides.length === 0 && !uploading && (
				...
				>
					...
					<p className="text-sm text-neutral-500 mb-6">
						Поддерживаются PowerPoint (.pptx) и PDF до 100MB
					</p>
					<button
					...
					<div className="grid grid-cols-3 sm:grid-cols-4 lg:grid-cols-6 xl:grid-cols-8 gap-3 mb-6">
						{parsedSlides.map(slide => (
							...
							>
								...
								<div className="absolute bottom-1.5 left-1.5 text-white text-xs">
									{slide}
								</div>
							</button>
						))}
					...
					<div className="bg-white rounded-xl p-4 sm:p-6 border border-neutral-200 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
						<div className="flex items-center gap-3">
							...
							<div className="text-sm">
								{selectedSlides.size} из {parsedSlides.length} слайдов выбрано
							</div>
						</div>
