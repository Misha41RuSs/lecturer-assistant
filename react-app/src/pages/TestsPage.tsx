...
interface ApiAnswer {
  ...
  questionType: "MULTIPLE" | "OPEN"; selectedOptionId: string | null;
  selectedOptionText: string | null; openText: string | null;
  score: number | null; maxScore: number; correct: boolean | null;
}
interface ApiSubmission {
  submissionId: string; chatId: number; startedAt: string;
  completedAt: string | null; totalScore: number; maxScore: number;
  hasUngraded: boolean; answers: ApiAnswer[];
}

interface DraftAnswer { id: number; text: string; correct: boolean }
interface DraftQuestion {
  id: number; text: string; type: "multiple" | "open";
  time: string; answers: DraftAnswer[];
}

function getStatusLabel(status: string) {
  if (status === "ACTIVE") return { text: "Запущен", cls: "bg-green-100 text-green-700" };
  if (status === "CLOSED") return { text: "Закрыт", cls: "bg-neutral-100 text-neutral-500" };
...
export function TestsPage() {
  ...
  if (view === "student-detail" && examDetail && selectedChatId !== null) {
    ...
    if (!sub) { setView("stats"); return null; }
    ...
    return (
      <div className="p-4 sm:p-6 lg:p-8">
        ...
        <p className="text-sm text-neutral-500 mb-6">
          ...
          {sub.hasUngraded && <span className="text-orange-500 ml-2">Есть непроверенные ответы</span>}
        </p>
                ...
                <div className="flex items-start gap-3 mb-3">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex gap-2 text-xs text-neutral-500">
                      ...
                      {q.timeLimitSec && <span className="flex items-center gap-0.5"><Clock className="w-3 h-3" />{q.timeLimitSec}с</span>}
                    </div>
                  </div>
                  {ans && (
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-2">
                        <input
                          ...
                          onBlur={e => {
                            const val = Math.min(ans.maxScore, Math.max(0, parseInt(e.target.value) || 0));
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                ...
                <tbody>
                  {submissions.map((r, i) => (
                    <tr key={i}
                      onClick={() => { setSelectedChatId(r.chatId); setView("student-detail"); }}
                      ...
                      <td className="py-2.5 px-3 text-sm flex items-center gap-2">
                        Студент #{r.chatId}
                        {r.hasUngraded && <span className="w-2 h-2 bg-yellow-500 rounded-full flex-shrink-0" />}
                      </td>
                      <td className="py-2.5 px-3 text-sm">{r.totalScore}/{r.maxScore}</td>
                      <td className="py-2.5 px-3">
                        {r.hasUngraded
                          ? <span className="text-xs px-2 py-0.5 rounded-full bg-yellow-100 text-yellow-700">На проверке</span>
                          : <span className={`text-xs px-2 py-0.5 rounded-full ${r.maxScore > 0 && r.totalScore >= r.maxScore * 0.7 ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"}`}>
                              {r.maxScore > 0 && r.totalScore >= r.maxScore * 0.7 ? "Сдал" : "Не сдал"}
                            </span>}
                      </td>
                    </tr>
                  ))}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr,300px] gap-6">
          <div className="space-y-4">
            <div className="bg-white rounded-xl p-5 border border-neutral-200">
              ...
              <div>
                <label className="block text-sm mb-1.5">Общее время на тест (мин), пусто = без ограничения</label>
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8">
      ...
      <div className="mb-6">
        <select value={selectedLectureId} onChange={e => setSelectedLectureId(Number(e.target.value))}
      ...
      {selectedLectureId > 0 && !loadingExams && exams.length === 0 && (
        <div className="bg-white rounded-xl p-12 border border-neutral-200 text-center">
          <p className="text-neutral-500 mb-4">Нет тестов для этой лекции. Создайте первый!</p>
                  ...
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="text-left min-w-0">
                      <div className="text-sm truncate flex items-center gap-2">
                        {exam.title}
                        <span className={`text-xs px-1.5 py-0.5 rounded-full ${status.cls}`}>{status.text}</span>
                      </div>
                      <div className="text-xs text-neutral-500">
                        {exam.questionCount ?? 0} вопросов
                        {exam.totalTimeSec && <> &middot; {Math.round(exam.totalTimeSec / 60)} мин.</>}
                      </div>
                    </div>
                  </div>
                ...
                {expandedExam === exam.id && (
                  <div className="border-t border-neutral-200 p-4">
                    <div className="flex flex-wrap gap-2 pt-1">
                      ...
                      {exam.status === "ACTIVE" && (
                        <button onClick={() => handleClose(exam.id)}
                          className="flex items-center gap-1 px-3 py-1.5 border border-neutral-300 rounded-lg text-sm hover:bg-neutral-50">
                          Закрыть тест
                        </button>
                      )}
