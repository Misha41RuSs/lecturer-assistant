...
import { Save, Play, Eye, EyeOff, Copy, QrCode, Lock, Globe, Mail } from "lucide-react";

export function LectureSettingsPage() {
  const navigate = useNavigate();
  const [lectureName, setLectureName] = useState("Введение в алгоритмы");
  const [description, setDescription] = useState("Обзор основных алгоритмов сортировки, поиска и анализа сложности.");
  ...
  const handleSave = () => {
    if (!lectureName.trim()) { toast.error("Введите название лекции"); return; }
    if (accessType === "password" && !password.trim()) { toast.error("Введите пароль"); return; }
    toast.success("Настройки сохранены");
    navigate("/my-lectures");
  };

  const handleStart = () => {
    if (!lectureName.trim()) { toast.error("Сначала заполните название"); return; }
    if (accessType === "password" && !password.trim()) { toast.error("Задайте пароль для лекции"); return; }
    ...
    <div className="p-4 sm:p-6 lg:p-8">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        ...
        <div className="flex gap-2">
          <button onClick={handleSave} className="flex items-center gap-2 px-4 py-2 border border-neutral-300 rounded-lg hover:bg-neutral-50 text-sm">
            <Save className="w-4 h-4" /> Сохранить
          </button>
          <button onClick={handleStart} className="flex items-center gap-2 px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 text-sm">
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-xl p-5 border border-neutral-200">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {slidePreview.map((s) => (
                <div key={s.id} className={`aspect-video bg-gradient-to-br ${s.color} rounded-lg p-3 flex flex-col justify-end`}>
            ...
            <div className="space-y-3 mb-4">
              {([
                ...
                <label key={opt.value}
                  ...
                  <input type="radio" name="access" value={opt.value} checked={accessType === opt.value}
                    onChange={() => setAccessType(opt.value)} className="w-4 h-4 accent-orange-500 mt-0.5" />
              ...
              <div className="p-4 bg-orange-50 border border-orange-200 rounded-lg">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <button onClick={() => setShowQR(!showQR)}
                    ...
                    <QrCode className="w-4 h-4" /> {showQR ? "Скрыть QR" : "Показать QR"}
                  </button>
                </div>
                <div className="flex gap-2 mb-3">
                  <input type="text" value={lectureUrl} readOnly
                    className="flex-1 px-3 py-2 bg-white border border-orange-300 rounded-lg text-sm" />
                  <button onClick={() => copyToClipboard(lectureUrl)}
                    className="px-3 py-2 border border-orange-300 rounded-lg hover:bg-orange-100 text-sm flex items-center gap-1">
        ...
        <div className="space-y-6">
          <div className="bg-white rounded-xl p-5 border border-neutral-200">
            ...
            <div className="mb-4">
              ...
              <select value={startSlide} onChange={(e) => setStartSlide(e.target.value)}
                className="w-full px-4 py-2.5 bg-neutral-50 border border-neutral-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500">
                {slidePreview.map((s) => (
                  <option key={s.id} value={s.id}>Слайд {s.id} — {s.title}</option>
                ))}
              </select>
            </div>
            <div className="mb-4">
          ...
          {accessType === "invitation" && (
            <div className="bg-orange-500 text-white rounded-xl p-5 text-center">
              ...
              <p className="text-orange-100 text-xs mt-2">Выведите на проектор для студентов</p>
            </div>
          )}
        </div>
      </div>
