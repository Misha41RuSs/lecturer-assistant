...
export function HomePage() {
	...
	const finishedCount = lectures.filter(
	...
	return (
		<div className="p-4 sm:p-6 lg:p-8">
			<div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
				<div>
					...
					<p className="text-sm text-neutral-500">
						Управляйте и проводите ваши лекции
					</p>
				</div>
				<Tooltip>
			...
			<div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-6">
				...
				<div className="bg-white rounded-xl p-4 lg:p-6 border border-neutral-200">
					<div className="text-2xl lg:text-3xl mb-1 text-orange-500">
						{runningCount}
					</div>
			...
			{loading ? (
				...
				<div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
					{filtered.map(lecture => (
						<div
							key={lecture.id}
							className="bg-white rounded-xl p-5 border border-neutral-200 relative"
						>
							<div className="flex items-start justify-between mb-3">
								...
								<div className="relative flex-shrink-0">
									<button
										onClick={() =>
											setMenuOpen(menuOpen === lecture.id ? null : lecture.id)
										}
									...
									{menuOpen === lecture.id && (
										<div className="absolute right-0 top-full mt-1 bg-white border border-neutral-200 rounded-lg shadow-lg z-10 w-48 py-1">
											<Tooltip>
												<TooltipTrigger asChild>
													<Link
														to={`/settings/${lecture.id}`}
														onClick={() => setMenuOpen(null)}
														className="flex items-center gap-2 w-full px-3 py-2 text-sm hover:bg-neutral-50"
													>
														<Settings className="w-3.5 h-3.5" /> Настройки
													</Link>
												</TooltipTrigger>
												<TooltipContent>
													<p>Изменить параметры лекции</p>
												</TooltipContent>
											</Tooltip>
											<Tooltip>
												<TooltipTrigger asChild>
							...
							<div className="flex gap-2">
								...
								{lecture.status === 'ACTIVE' ? (
									<Tooltip>
										...
										<TooltipContent>
											<p>Перейти к живой презентации</p>
										</TooltipContent>
									</Tooltip>
								) : (
									<Tooltip>
										...
										<TooltipContent>
											<p>Настроить параметры лекции</p>
										</TooltipContent>
									</Tooltip>
								)}
							</div>
