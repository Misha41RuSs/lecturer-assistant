...
interface DrawAction {
  ...
  start?: Point;          // rect / ellipse / arrow
  end?: Point;
  text?: string;          // text
  position?: Point;
}
...
export function DrawingOverlay({ slideIndex, active, onToggle }: Props) {
  ...
  // Per-slide annotations stored by slideIndex
  ...
  const drawAction = useCallback((ctx: CanvasRenderingContext2D, action: DrawAction, w: number, h: number) => {
  ...
  const redraw = useCallback(() => {
    ...
    const acts = allAnnotations[slideIndex] || [];
    for (const action of acts) {
      drawAction(ctx, action, canvas.width, canvas.height);
    }
  }, [slideIndex, allAnnotations, drawAction]);

  useEffect(() => { redraw(); }, [redraw]);

  const handlePointerDown = (e: React.MouseEvent) => {
    if (!active) return;
    if (textInput) return; // text input active
    const pos = getPos(e);
  ...
  const handlePointerMove = (e: React.MouseEvent) => {
    ...
    if (tool === "pen" || tool === "eraser") {
      ...
      // Redraw all + preview shape
      redraw();
      drawAction(ctx, { tool, color, size, start: startPoint, end: pos }, canvas.width, canvas.height);
    }
  };
  ...
  const handlePointerUp = () => {
    ...
    if (tool === "pen" || tool === "eraser") {
      if (currentPoints.length > 0) {
        setAnnotations([...annotations, { tool, color, size, points: currentPoints }]);
      }
      setCurrentPoints([]);
    } else if (startPoint) {
      setStartPoint(null);
  ...
  // For shape tools, capture end on mouseup via a separate handler
  ...
  const handleSaveAnnotations = () => {
    ...
    if (totalAnnotations === 0) { toast.info("Нет рисунков для сохранения"); return; }
    toast.success(`Рисунки сохранены (${totalAnnotations} элементов на ${Object.keys(allAnnotations).filter(k => (allAnnotations[parseInt(k)] || []).length > 0).length} слайдах). Студенты получат презентацию с аннотациями.`);
  };

  const handleExportSlide = () => {
    const canvas = canvasRef.current;
    if (!canvas) return;
    // Create a composite: we just export the annotation layer
  ...
  return (
    <>
      ...
      {/* Save/export bar below canvas when drawing mode active */}
      {active && (
