...
**Появление слайда (системное событие)**
```json
{
  "verb": "slide_shown",
  "lectureId": 1,
...
## Как считаются метрики понятности

### Clarity Rating (CR) - Оценка понятности
...
Где:
- `rating_i` — оценка понятности от каждого слушателя (1–5)
- `N` — количество оценок

**Пример**: если 4 слушателя оценили слайд в 4, 5, 4, 3 балла, то CR = (4+5+4+3)/4 = 4.0

### Question Density (QD) - Плотность вопросов
...
Где:
- `Q` — количество вопросов (verb='asked')
- `S` — количество уникальных слайдов в лекции
- `U` — количество уникальных слушателей, задавших хотя бы один вопрос

**Пример**: если 6 вопросов, 3 слайда, 2 активных слушателя, то QD = 6/(3×2) = 1.0

### Question Temporal Depth (QTD) - Время от слайда до вопроса
...
Где:
- `t_question` — время события вопроса
- `t_slide` — время появления слайда перед вопросом (для того же slideId)
- `M` — количество вопросов, для которых найден соответствующий слайд

**Алгоритм**: для каждого вопроса находим последнее событие «слайд показан» с тем же slideId, вычисляем разницу во времени (в секундах), затем усредняем.

**Пример**: если вопрос задан через 30 сек после показа слайда, а второй вопрос через 90 сек, то QTD = (30+90)/2 = 60.0 сек
